...
const RoyalButton = forwardRef<HTMLButtonElement, RoyalButtonProps>(({
  ...
  return <button ref={ref} className={`${baseStyles} ${variants[variant]} ${sizes[size]} ${className}
        ...
        <span className="absolute inset-[1px] rounded-full pointer-events-none
            bg-gradient-to-b from-white/8 via-transparent to-transparent" aria-hidden="true" />
        
        {/* Text with drop shadow for readability */}
        <span className="relative z-10 drop-shadow-[0_1px_2px_rgba(0,0,0,0.3)] mx-0 text-center font-mono">
          {children}
        </span>
      </button>;
});
