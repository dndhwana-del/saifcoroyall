  ...
  return <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0">
        <AnimatePresence mode="wait">
          {heroSlides.map((slide, index) => index === currentSlide && <motion.div key={index} className="absolute inset-0" initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} exit={{
      ...
      {heroSlides.length > 1 && <div className="absolute bottom-32 left-1/2 -translate-x-1/2 z-20 flex items-center gap-3">
          {heroSlides.map((_, index) => <button key={index} onClick={() => setCurrentSlide(index)} className={`w-2 h-2 rounded-full transition-all duration-500 ${index === currentSlide ? 'bg-gold w-8' : 'bg-sand/40 hover:bg-sand/60'}`} aria-label={`Go to slide ${index + 1}`} />)}
        </div>}
      ...
      <div className="relative z-20 container mx-auto px-6 text-center">
        <div className="max-w-4xl mx-auto">
          ...
          }} className="relative text-5xl md:text-7xl lg:text-8xl font-royal leading-tight uppercase tracking-[0.15em]" style={{
            textShadow: '0 4px 30px rgba(0,0,0,0.6), 0 2px 10px rgba(0,0,0,0.4)'
          }}>
              <span className="royal-title-embossed text-right text-7xl font-thin font-serif">Royal Gulf</span>
            </motion.h1>
          ...
          duration: 0.8,
          delay: 0.6
        }} className="font-display text-xl md:text-2xl text-sand/90 mb-4 italic" style={{
          ...
          duration: 0.8,
          delay: 0.8
        }} className="font-body text-lg md:text-xl max-w-2xl mx-auto mb-10 leading-relaxed text-gray-100" style={{
          textShadow: '0 2px 15px rgba(0,0,0,0.6)'
        }}>
